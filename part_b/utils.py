...
def load_question_meta(num_questions, num_subjects, root_dir="/data") -> np.array:
    """
    ...
    question_meta = pd.read_csv(path).to_numpy()
...
def load_student_meta(num_students, root_dir="/data") -> np.array:
    """
    Process the question metadata
    Output is a numpy array of shape (num_questions, num_subjects)
    where arr[i, j] is 1 if question i is of subject j and 0 otherwise
    ...
    student_meta = pd.read_csv(path).to_numpy()
    meta_mat = np.zeros((num_students, 3)) #two values per student: sex, age, type of student
    for question, sex, age, premium in student_meta:
        if type(age) == str:
            age = 2024 - int(age[:4])
        meta_mat[question] = [sex, age, premium]
    return meta_mat
