"""
This file has a modified version of the AutoEncoder where questions were used as the entity.
"""
...
class AutoEncoder(nn.Module):
    def __init__(self, num_input, g = 50, h = 50, question=False):
        ...
        self.h = nn.Linear(h + 1, num_input)
    ...
    def forward(self, inputs, beta):
        ...
        encode = torch.cat((encoded, betas), dim=1)
...
def train(model, lr, lamb, train_data, zero_train_data, valid_data, num_epoch, eta, question=False):
    ...
    for epoch in range(0, num_epoch):
        ...
        for user_id in range(num_student):
            ...
            target = inputs.clone()
            optimizer.zero_grad()
            output = model(inputs, eta[user_id])
            ...
            optimizer.step()
        valid_acc = evaluate(model, eta, zero_train_data, valid_data, question)
...
def evaluate(model, eta, train_data, valid_data, question=False):
...
def main():
    ...
    train(model, 0.1, 0.001,  train_matrix,  zero_train_matrix, valid_data, 5, eta, question=question)
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
